...
export async function getComboStock(tenantId: string, query: string) {
  ...
  if (!components || components.length === 0) {
    return {
      ...
      message: `El producto "${product.title}" no está configurado con componentes en LibretaX. Su stock publicado en Mercado Libre es de ${product.available_quantity} unidades.`
    };
  }
