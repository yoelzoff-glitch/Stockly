...
export async function getCoupons(tenantId: string) {
  ...
  try {
    const response = await meliFetch({
      tenantId,
      endpoint: MELI_PROMOTION_ENDPOINTS.GET_COUPONS(account.meli_user_id),
      method: "GET"
    });
    return response;
  } catch (error: any) {
    if (error.statusCode === 403 || error.statusCode === 404) {
      throw new AppError("VALIDATION_ERROR", "Mercado Libre no permite consultar cupones para tu cuenta.", error.statusCode);
    }
    throw error;
  }
}
