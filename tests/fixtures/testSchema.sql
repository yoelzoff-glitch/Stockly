...
CREATE TABLE IF NOT EXISTS public.subscriptions (
  ...
  monthly_price_snapshot numeric NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS public.usage_events (
  ...
  status text NOT NULL DEFAULT 'applied' CHECK (status = ANY (ARRAY['applied'::text, 'rejected'::text, 'duplicate'::text])),
  source text,
  correlation_id text,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT usage_events_pkey PRIMARY KEY (id),
  CONSTRAINT usage_events_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES public.tenants(id) ON DELETE CASCADE,
...
-- Sprint 12: RPC transaccional para upsert con dedupe_key (evita error 42P10)
